...
/**
 * This is a restful controller which accepts the API calls for adding, updating, deleting and
 * viewing the records from the report_master table.
 */
@RestController
...
public class ReportMasterController {
  ...
  public ResponseEntity<?> createReport(
    ...
    try {
      ...
      return new ResponseEntity(
          "Unable to add the new record due to the following error  " + e.getMessage(),
  ...
  public ResponseEntity<?> updateRecord(@PathVariable("id") final String id,
    ...
    try {
      ...
      return new ResponseEntity("Unable to update. Record with report name " + id,
          HttpStatus.INTERNAL_SERVER_ERROR);
